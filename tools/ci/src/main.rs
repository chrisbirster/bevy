...
fn main() {
    ...
    if what_to_run.contains(Check::CLIPPY) {
        ...
        cmd!("cargo clippy --workspace --all-targets --all-features -- -A clippy::type_complexity -W clippy::doc_markdown -W clippy::option_if_let_else -W clippy::redundant_else -D warnings")
        .run()
